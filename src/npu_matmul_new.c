...
void gen_matmul_task(uint64_t *ops, npu_cna_desc *cna_desc, npu_core_desc *core_desc, npu_dpu_desc *dpu_desc) {
  ...
  value = ((uint32_t)dpu_desc->notch_addr & 0xFFFF);
  value = value | (value << 16);
  ops[61] = NPUOP(OP_REG_DPU, value, DPU_DATA_CUBE_NOTCH_ADDR);
...
int gen_matmul_fp16(matmul_params_t *params) {
   ...
   int align_in = ((params->k + 31) / 32) * 32;
   if (align_in < 32) align_in = 32;
   int align_out = ((params->n + 31) / 32) * 32;
   if (align_out < 32) align_out = 32;
   const int is_matmul_64 = (params->m == 64 && params->n == 64 && params->k == 64);
   const int is_matmul_256 = (params->m == 256 && params->n == 256 && params->k == 256);

   cna_desc.conv_mode = direct_convolution;
   cna_desc.in_precision = precision_float16;
   cna_desc.proc_precision = precision_float16;
   cna_desc.group_line_off = (uint8_t)(!is_matmul_64 && !is_matmul_256);
  ...
  //  cna_desc.datain_channel = align_in;
  ...
  //  cna_desc.data_entries = (cna_desc.datain_width * align_in) / 32;
   cna_desc.data_entries = (((cna_desc.datain_width * cna_desc.datain_channel) % 32) == 0) ? 
  //  cna_desc.data_entries = (((cna_desc.datain_width * align_in) % 32) == 0) ? 
  ...
  //  surf_stride = 0;
  //  if (align_in >= 64 && cna_desc.datain_height >= 4) {
  //    surf_stride = cna_desc.line_stride * ((cna_desc.datain_height / 4) - 1);
  //  }
  ...
  //  dpu_desc.dst_surf_stride = (uint32_t)(is_matmul_64 ? 64 : (is_matmul_256 ? 256 : 1));
  ...
  //    dpu_desc.surf_add = (!params->fp32tofp16) ? dpu_desc.dst_surf_stride * 4 : dpu_desc.dst_surf_stride * 2;
  //  } else {
  //    uint32_t stride_units = params->fp32tofp16 ? (uint32_t)(align_out / 16u) : (uint32_t)(align_out / 8u);
  //    if (stride_units == 0) stride_units = 1;
  //    dpu_desc.surf_add = dpu_desc.dst_surf_stride * stride_units;
  //    if (!params->fp32tofp16 && align_out == 64) {
  //      dpu_desc.surf_add = dpu_desc.dst_surf_stride * 4;
  //    }
  //  }

  //  dpu_desc.notch_addr = 7;
  //  if (is_matmul_64 || is_matmul_256) {
  //    dpu_desc.notch_addr = 0;
  //  } else if (cna_desc.dataout_height == 64) {
  //    dpu_desc.notch_addr = 15;
  //  }
