...
int main(int argc, char **argv) {
  ...
  size_t output_bytes = output_elems * sizeof(_Float16);
  ...
  for (int m = 1; m <= M; m++) {
    for (int k = 1; k <= K; k++) {
      feature_data_fp16[feature_data(align_in, M, 1, align_in, k, m, 1)] =
        matrixA[((m-1)*K)+(k-1)];
    }
  }
  ...
  for (int m=1;m<=M;m++) {
    for (int n=1;n<N;n++) {
      _Float16 actual = output_data[feature_data(align_out, M, 1, align_out, n, m, 1)];
